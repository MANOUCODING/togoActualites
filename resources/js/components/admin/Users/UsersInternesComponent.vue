				...
				<div class="card border bg-transparent rounded-3">
					...
					<div class="card-header bg-transparent border-bottom p-3">
						<div class="d-sm-flex justify-content-between align-items-center">
							<h5 class="mb-2 mb-sm-0">Publicateurs d'articles <span class="badge bg-primary bg-opacity-10 text-primary">{{ infos.usersCountHH  }}</span></h5>
						</div>
					</div>
					<!-- Card header END -->
                    ...
                    <th scope="col" class="border-0">Ajouté le </th>
								...
								<!-- Table body START -->
								<tbody class="border-top-0" v-for="info in infos.newsLattersHH.data" :key="info.id" >
									<!-- Table item -->
									<tr>
										<!-- Table data -->
										<td>
    ...
    changeStatusLock(id) {
       this.$swal({
        ...
        if (confirmed.isConfirmed) {
          axios.patch(`/api/users/${id}/status/HH`)
          .then(response => {
              this.getResults();

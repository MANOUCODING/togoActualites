...
<section class="py-4">
    ...
    <div v-else class="row pb-4">
			<div class="col-12">
        <!-- Title -->
					<h1 class="mb-0 h2">Modifier un  article</h1>
			</div>
		</div>
            ...
            <form action="" method="POST" enctype="multipart/form-data">
              ...
              <!-- Main toolbar -->
                ...
                <div class="col-md-12 text-start"> <br>
                  <button class="btn btn-primary w-100" v-if="!loadingSave" type="submit" @click.prevent="update()">Modifier cet article</button>
                   <button class="btn btn-primary w-100" v-else type="button">Modification de l'article en cours ...</button>
                </div>
              </div>
            </form>
            <!-- Form END -->
...
export default{
  ...
  methods: {
    getResults(){
      axios
        .get(`/api/article/${this.$route.params.slug}/show`)
        .then(response => {
          this.load = false
    ...
    fieldChange(e){
     ...
     update(){
      ...
      axios
        .put(`/api/articles/${this.$route.params.slug}/update`, this.form, config)
        .then(response => {
          console.log(response.data)
          if(response.status == 200){
            ...
            if (response.data.success == false) {
              ...
              if (response.data.message == 'L\'article a été modifié avec succès.' ) {
                this.$swal({
                  title: "Succès!",
